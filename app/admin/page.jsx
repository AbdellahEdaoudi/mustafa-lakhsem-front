...
import { Loader2, Star, Mail, Trash2 } from "@/components/lucide-react";
      ...
      <header className="sticky top-0 z-30 border-b border-slate-800/80 bg-[#030712]/90 backdrop-blur-xl transition-all">
        <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 h-16 sm:h-20 flex items-center justify-between gap-2 sm:gap-4">
          <div className="flex items-center gap-2 sm:gap-4 min-w-0 shrink">
            <Link
              href={"/" + lang}
              className="flex items-center gap-2 sm:gap-3 group transition-transform duration-300 hover:scale-105 cursor-pointer min-w-0"
            >
              ...
              <div className="relative shrink-0" style={{ width: 40, height: 40 }}>
                ...
                >
                  <GoldVerifiedBadge size={17} />
                </span>
              </div>
              <div className="flex flex-col min-w-0">
                <span className="text-xs xs:text-sm sm:text-base md:text-lg font-black tracking-wider text-[#f8fafc] group-hover:text-[#d4af37] transition-colors truncate">
                  {t?.dashboard?.brandName || "MUSTAFA LAKHSEM"}
                </span>
                <span className="text-[9px] sm:text-[10px] tracking-widest text-[#d4af37] uppercase font-semibold truncate">
          ...
          <div className="flex items-center gap-1.5 sm:gap-3 shrink-0">
            ...
            <button
              ...
              className="p-1.5 sm:p-2 text-slate-400 hover:text-white rounded-xl hover:bg-slate-800/60 border border-transparent hover:border-slate-700/60 transition-all cursor-pointer disabled:opacity-50"
            >
            ...
            <button
              ...
              disabled={loggingOut}
              className="flex items-center gap-1.5 sm:gap-2 px-2.5 sm:px-3.5 py-1.5 sm:py-2 text-xs font-semibold bg-red-500/10 hover:bg-red-500/20 text-red-400 hover:text-red-300 border border-red-500/20 hover:border-red-500/40 rounded-xl transition-all cursor-pointer disabled:opacity-50"
            >
              {loggingOut ? (
            ...
            {activeTab === "contacts" && (
              <div>
                ...
                {filteredContacts.length === 0 ? (
                  ...
                  <div>
                    {/* Mobile View: Ultra-Compact Cards (Visible on mobile screens < md) */}
                    <div className="flex flex-col gap-2 md:hidden">
                      {filteredContacts.map((c) => (
                        <div
                          key={c._id}
                          onClick={() => openContact(c)}
                          className={`p-2.5 rounded-xl border transition-all cursor-pointer flex flex-col gap-1.5 relative ${!c.isRead
                        ...
                        >
                          {/* Header Row: Status Dot + Name + Topic Tag + Date */}
                          <div className="flex items-center justify-between gap-2 min-w-0">
                            <div className="flex items-center gap-2 min-w-0 flex-1">
                              <span
                                className={`w-2 h-2 rounded-full shrink-0 ${!c.isRead ? "bg-amber-400 shadow-xs shadow-amber-400" : "bg-slate-700"
                                  }`}
                              />
                              <span className={`text-xs min-w-0 truncate ${!c.isRead ? "font-bold text-white" : "font-medium text-slate-200"}`}>
                                {c.name}
                              </span>
                              {c.type && (
                                <span className="px-1.5 py-0.5 rounded text-[10px] font-semibold bg-amber-500/10 text-amber-400 border border-amber-500/20 truncate shrink-0 max-w-32.5">
                                  {c.type}
                                </span>
                              )}
                            </div>
                          ...
                          {/* Row 2: Message preview (1 line snippet) */}
                          <p
                            dir={isArabic(c.message) ? "rtl" : "ltr"}
                            className="text-[11px] text-slate-400 truncate leading-snug ps-4"
                          >
                            {c.message}
                          </p>

                          {/* Row 3: Email & Compact Actions */}
                          <div className="flex items-center justify-between gap-2 text-[10px] pt-0.5 ps-4">
                            <span className="truncate font-mono text-slate-500 min-w-0">{c.email}</span>
                            ...
                            <div className="flex items-center gap-1.5 shrink-0" onClick={(e) => e.stopPropagation()}>
                              <button
                                ...
                                className={`p-1 rounded-lg border transition-all cursor-pointer disabled:opacity-50 ${c.isStarred
                                  ? "border-amber-500/40 text-amber-400 bg-amber-500/15"
                                  : "border-slate-800 text-slate-400 hover:text-amber-400 bg-slate-900/60"
                                  }`}
                              >
                                {starringId === c._id ? (
                                  ...
                                  <Star
                                    ...
                                    stroke={c.isStarred ? "#f59e0b" : "currentColor"}
                                  />
                                )}
                              </button>
                              ...
                              <button
                                ...
                                className={`p-1 rounded-lg border transition-all cursor-pointer disabled:opacity-50 ${c.isRead
                                  ? "border-slate-800 text-slate-400 hover:text-amber-400 bg-slate-900/60"
                              ...
                              <button
                                ...
                                className="p-1 rounded-lg border border-red-500/20 text-red-400 hover:bg-red-500/20 hover:text-red-300 bg-red-500/10 transition-all cursor-pointer"
                              >
